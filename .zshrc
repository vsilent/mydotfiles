...
if [[ $WMII_ADD = "" ]]; then
  ...
  export PATH="/opt/java/jre/bin/:/java/bin:$PATH"
  VIM=/usr/bin/vim
  VIMRUNTIME=~/.vim
  EDITOR=$VIM
  SVN_EDITOR=$VIM
...
#alias cc "./symfony cc"
alias replaceroot='find -type f -iname "Root" -exec sed -i "s/192.168.1.1/192.168.1.2/g" {} \;'
alias back='cd "$OLDPWD";pwd  '
alias vconf='sudo vim /etc/httpd/conf/extra/httpd-vhosts.conf'
alias vhost='sudo vim /etc/apache2/sites-available/httpd-vhosts'
...
abbreviations=(
  ...
  "g" "grep 'indexAction\(' ~/work/proj/**/*.php |vim -"
  "gr" "grep 'indexAction\(' ~/work/proj/**/*.php |vim -"
  "grep" "grep -HEnq 'indexAction\(' ~/work/proj/**/*.php| vim -"
  "res" 'svn resolve --accept working yourconflictdir'
  "svr" 'svn resolve --accept working yourconflictdir'
  ...
  "c" 'php app/console'
  "s" 'php app/console'
  "sr" 'php app/console router:debug'
  "dmi" 'php app/console doctrine:mapping:info'
  "dsc" 'php app/console doctrine:schema:create'
  "ddc" 'php app/console doctrine:database:create -e=dev'
  "ddd" 'php app/console doctrine:database:drop'
  "dsu" 'php app/console doctrine:schema:update --force'
  "dcc" 'php app/console doctrine:cache:clear-metadata'
  "sgb" 'php app/console generate:bundle --namespace=DriveBro\StoreBundle'
  "dges" 'php app/console doctrine:generate:entities DriveBro\Entity\User'
  "dge" 'php app/console generate:doctrine:entity --entity="DriveBroStoreBundle:Product" --fields="name:string(255) price:float description:text"'
  "ge" 'php app/console generate:doctrine:entity --entity="DriveBroStoreBundle:Product" --fields="name:string(255) price:float description:text"'
  "gsub" 'git submodule -f add git://github.com/l3pp4rd/DoctrineExtensions.git vendor/gedmo-doctrine-extensions'
  "gir" 'git rebase -i'
  "hotlist" "gvim ~/.mc/hotlist"
  "hs" "find $SRC_DIR -name '*.php' | grep -v phpunit > $OUTPUT_DIR/files.txt"
  "h" "$HPHP_HOME/src/hphp/hphp \
  --input-list=$OUTPUT_DIR/files.txt \
  --input-dir=$SRC_DIR \
  --cmodule=$SRC_DIR/htdocs \
  ...
  'sm' 'php symfony doctrine:migrate --up'
  'sbc' 'php symfony doctrine:build --all-classes'
  'bac' 'php symfony doctrine:build --all-classes'
  'sbac' 'php symfony doctrine:build --all-classes'
  ...
  "nm" "nmap -vv -sP 192.168.1.0/24"
  "hot" "gvim ~/.mc/hotlist"
  "p" "sudo ps aux | grep "
  "pa" "sudo ps aux | grep "
  "ps" "sudo pacman-color -Ss "
  "pu" "sudo pacman-color -Syu "
  "phpunit" "phpunit --group signup --verbose --stop-on-failure"
  'pgd' 'pg_dump -h localhost -W -U user database > dump.sql'
  'pg' 'psql -U user database < dump.sql'
  "pi" "sudo pacman-color -S "
  "yu" "yaourt -Syu "
  "yi" "yaourt -S "
  "ys" "yaourt -Ss "
  "yr" "yaourt -R "
  ...
  "sco" "svn checkout svn+ssh://vs@src/var/lib/svn/projectname/trunk projectname"
  "svi" 'svn import -m "Initial commit" svn+ssh://vs@src/var/lib/svn/projectname/trunk projectname'
  "sui"  "sudo aptitude install"
  "sadu" "sudo aptitude dist-upgrade"
  "sdu"  "sudo aptitude dist-upgrade"
  "sd"	 "svn diff --diff-cmd=meld "
  "svnd" "svn diff -r 63:64 --diff-cmd=meld "
  "svnr" "svn diff -r 63:64 --diff-cmd=meld "
  "su"	 "svn update"
  "sur"  "sudo aptitude remove"
  "sc"	 'svn commit -m "'
  "scc1"   './symfony cc'
  "sb"	 'php app/console generate:bundle --namespace=Acme/HelloBundle --format=yml'
  'sl' "php symfony i18n:extract frontend ru --auto-save"
  'smod' 'php symfony doctrine:generate-module --with-show --non-verbose-templates frontend news RudcomNews'
  'smodule' 'php symfony doctrine:generate-module --with-show --non-verbose-templates frontend news RudcomNews'
  'splugin' 'php symfony doctrine:generate-module --with-show --non-verbose-templates frontend news RudcomNews'
  'splug' 'php symfony plugin:install '
  'slang' "php symfony i18n:extract frontend ru --auto-save"
  'sm' 'php symfony doctrine:migrate --up'
  'sbc' 'php symfony doctrine:build --all-classes'
  'bac' 'php symfony doctrine:build --all-classes'
  'sbac' 'php symfony doctrine:build --all-classes'
  'scc' 'php app/console cache:clear --env=prod --no-debug'
  'sys' 'php app/console container:debug'
  'services' 'php app/console container:debug'
  "sbundle"  'php app/console generate:bundle --namespace=Acme/HelloBundle --format=yml'
  ...
  "gb"	 'git branch'
  "gdr"   'git push origin --delete <branchName>'
  "gd"	 'git branch -D <branchName>'
  "gp"	 'git pull'
  "gg"	 'git push'
  "gc"	 'git commit -m'
  "gl"	 'git checkout live'
  "g"	 'git log --pretty=format:"%h %ad | %s%d [%an]" --graph --date=default --decorate'
  ...
  "gco"  'git co '
  "gt"	 'git ticket 125'
  "gcl"	 'git clone git+ssh://vs@server/var/svn/repo.git'
  "db"	 'git push origin --delete <branchName>'
  "gdb"   'git push origin --delete <branchName>'
  ...
  "xi" 'xinput set-prop 13 "Device Enabled" 0'
  "scp" "scp filetocopy user@remoteserver:/var/www/vhosts/"
  "wmii"  "vim ~/.wmii-3.5/events.py"
  "wmi"    "vim ~/.wmii-3.5/events.py"
  'con' 'convert -quality 100 -font /path_to_font/arial.ttf -pointsize 40 /path_to_image/test.jpg -gravity "South" -draw "text -110,0 John Karter" /tmp/res.jpg'
  'usermod' "usermod -a -G www-data vs"
)
...
#function parse_git_branch () {
	#git branch 2>/dev/null|grep -e '* .*' -os|cut -d'*' -f2
#}
